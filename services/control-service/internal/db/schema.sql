...
CREATE TABLE IF NOT EXISTS credentials (
  ...
  delivery_mode text NOT NULL CHECK (delivery_mode IN ('server_only', 'client')),
...
CREATE TABLE IF NOT EXISTS license_activations (
  ...
  last_seen_at timestamptz NOT NULL DEFAULT now(),
  revoked_at timestamptz,
  UNIQUE (license_id, deployment_id)
);

CREATE INDEX IF NOT EXISTS idx_licenses_deployment_status ON licenses (deployment_id, status, expires_at);
CREATE INDEX IF NOT EXISTS idx_license_activations_deployment ON license_activations (deployment_id, license_id, revoked_at);
