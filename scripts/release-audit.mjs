...
const backupCompose = await readText("scripts/backup-compose.mjs");
const restoreCompose = await readText("scripts/restore-compose.mjs");
...
assert(packageDocument.scripts?.["test:backup-security"] === "node tests/backup-security.mjs", "backup security unit gate is not wired");
assert(backupCompose.includes("'--pull', 'never'"), "backup helper image may be pulled implicitly");
assert(restoreCompose.includes("'--pull', 'never'"), "restore helper image may be pulled implicitly");
assert(restoreCompose.includes("manifest.helperImage !== helperImage"), "restore trusts the backup manifest to select its helper image");
