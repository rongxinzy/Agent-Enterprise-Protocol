...
验证未签名的 base/gateway Bundle 待签输入，并发布源码和三个服务镜像的
CycloneDX SBOM、绑定提交的 release manifest 以及 SHA-256 校验文件。详见
[企业底座发布制品](release-artifacts.zh-CN.md)。该工作流不会接触离线 Bundle
签名器、License 签名器或任何私钥。客户离线 Bundle、Electron 安装包和 License
只能在批准的本地签名环境中生成并签名。
