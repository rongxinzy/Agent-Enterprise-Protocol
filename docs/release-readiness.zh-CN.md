...
GA 验证，5%：负载与长稳验证工作流，以及 PostgreSQL/MinIO 备份与灾备恢复演练均已完成。剩余工作是外部安全评审、正式客户制品签名与 SBOM 发布，以及草案档位的剩余清理。详见 [GA 验证手册](ga-validation-runbook.zh-CN.md) 和 [备份与灾备恢复手册](backup-restore-runbook.zh-CN.md)。M3 已通过管理 API 提供单个 Skill 版本撤回；撤回后禁止继续下载该版本，同时保留审计记录。
