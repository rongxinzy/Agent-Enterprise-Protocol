...
服务端必须从 access token claim 推导用户、部署和会话身份。客户端不得使用身份请求头替代
这些 claim。服务端应回传 `X-Request-ID`；客户端未提供时应生成一个。

条件请求使用标准 HTTP 请求头，例如 `ETag` 和 `If-None-Match`。

## 7. 身份认证

服务端返回当前部署启用的登录方式。AEP v1 生产档位支持：
...
`federated` 契约为未来的客户身份适配器保留。现有 mock 实现仅用于开发；在未配置真实
适配器时，生产服务端不得公布该能力。未来的 OIDC 适配器必须使用 Authorization Code +
PKCE，签发相同的短生命周期、单次使用交换码，且不得让客户密码经过 AEP。

每种已启用的登录方式最终建立相同的 AEP 会话，并返回：

- 调用管理 API 的 AEP access token；
- 用于会话续期的 refresh token；
- 调用模型网关的 model access token。

model access token 在登录和刷新时签发，包含明确的模型网关 audience、有效期、部署身份、
...
管控事件支持 `global`、`team`、`role` 和 `user` 四种作用域。服务端根据经过认证的
身份及其角色、团队绑定计算适用作用域，客户端不得自行选择角色、团队或用户作用域。

全局、团队、角色或用户事件必须为每个适用用户会话分别维护投递状态，事件本身不存在共享的
`consumed` 标志。用户离线时，处于 active 且未过期的事件仍会保留；后续建立会话时，服务端
为该会话创建适用的 delivery。
...
管理端 API 管理 Skill 及其版本、用户、角色或团队授权、凭证及轮换、模型描述及授权，以及事件查询。
...
主版本号出现在基础路径中。在发布候选阶段及正式发布后，新增可选字段和端点不改变主版本，
客户端必须忽略未知 JSON 字段。破坏性变更必须使用新路径，例如 `/aep/v2`。
